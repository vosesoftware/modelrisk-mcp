...
If you see the transport / port / token options, the entry point is wired correctly. Beyond that, the fastest way to verify the full server works is to point MCP Inspector at it (`uv run mcp dev src/modelrisk_mcp/__main__.py`) and check that `tools/list` returns 47 tools — `list_open_workbooks`, `get_active_workbook`, `audit_model`, `run_simulation`, etc.
