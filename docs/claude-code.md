...
Opens the MCP Inspector at <http://localhost:5173>. You should see **43 tools** listed under `tools/list` — `list_open_workbooks`, `get_active_workbook`, `audit_model`, `run_simulation`, `build_drivers_report`, and so on. Every description starts with `"ModelRisk: ..."`.
