...
- **Doesn't replace ModelRisk's UI for everything.** ModelRisk has GUI flows for cross-section fitting, Bayesian network design, custom-distribution authoring that aren't exposed via MCP yet. The 50 tools cover the typical model-build → simulate → interpret loop; advanced flows still happen in the ribbon.
